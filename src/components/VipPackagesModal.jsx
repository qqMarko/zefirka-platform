...
const VipPackagesModal = ({ setShowVipModal, userRole, openWalletWithAmount }) => {
    ...
    // 🟢 СТЕЙТ ДЛЯ ВНУТРІШНЬОЇ МОДАЛКИ (ДЕТАЛІ ПАКЕТУ)
    ...
    const modelPackages = [
        {
            ...
            topFeatures: [
                'Яскраве виділення анкети', 
                '+5 безкоштовних підняттів в ТОП'
            ],
            features: [
                'Пріоритетне розміщення у пошуку', 
                'Розширена статистика переглядів', 
                'Можливість додати до 3-х анкет', 
                'Базова підтримка 24/7'
            ], 
            isPopular: false
        },
        {
            ...
            topFeatures: [
                '💎 Усі переваги статусу START', 
                'Ексклюзивна золота рамка'
            ],
            features: [
                '+10 безкоштовних підняттів в ТОП', 
                'Високий пріоритет у каталозі', 
                'Премодерація нових відгуків', 
                'Доступ до системи Арбітражу', 
                'До 5-ти активних анкет'
            ], 
            isPopular: true
        },
        {
            ...
            topFeatures: [
                '👑 Усі переваги статусу PREMIUM', 
                'Унікальна RGB-рамка анкети'
            ],
            features: [
                'Автоматичне підняття в ТОП', 
                'Абсолютний ТОП у видачі', 
                'Повне керування відгуками', 
                'Прямий VIP-чат з адміністрацією', 
                'До 10-ти активних анкет'
            ], 
            isPopular: false, isRgb: true
        }
    ];

    // 🟢 ОПТИМІЗОВАНИЙ КОПІРАЙТИНГ ДЛЯ КЛІЄНТІВ
    const clientPackages = [
        {
            ...
            topFeatures: [
                'Усі переваги статусу Безкоштовно',
                
            ],
            features: [
                'Вищий пріоритет в особистих повідомленнях моделі 🚀',
                'Додавання анкет в Обране 🩷', 
                'Доступ до розділу Арбітраж ⚖️',
            ], 
            isPopular: false
        },
        {
            ...
            topFeatures: [
                ' Усі переваги статусу GUEST', 
            ],
            features: [
                'Вищий пріоритет для розгляду арбітражних скарг 🚀',  
                'Виділення повідомлень кольором в чаті моделі 🎨',
                'Можливість залишити відгук на анкеті моделі 💬',
            ], 
            isPopular: true
        },
        {
            ...
            topFeatures: [
                '👑 Усі переваги статусу PRIORITY', 
            ],
            features: [
                'Персональний менеджер 24/7', 
                'Приховане прочитання повідомлень 😶‍🌫️',
                'Найвищий пріоритет для розгляду арбітражних скарг 🚀',
                'Відшкодування коштів 💵',
                'Персональне вирішення проблем через адміністратора 🤴'
            ], 
            isPopular: false, isRgb: true
        }
    ];
    ...
    const executePurchase = async (priceStr, packageId) => {
        ...
        if (balance >= amount) {
            ...
            try {
                ...
                toast.error('Мережева помилка. Перевірте консоль!', { id: loadingToast }); 
            } finally { 
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(5, 5, 10, 0.9)', backdropFilter: 'blur(25px)', cursor: 'pointer' }} onClick={() => setShowVipModal(false)} />
                    ...
                    {packages.map((pkg) => {
                        ...
                        return (
                            <div key={pkg.id} className={!isActive ? "micro-card" : ""} style={{ position: 'relative', background: isActive ? 'linear-gradient(145deg, rgba(20, 40, 20, 0.9), rgba(10, 20, 10, 0.9))' : 'rgba(10, 10, 15, 0.8)', borderRadius: '24px', border: `1px solid ${isActive ? '#4caf50' : (pkg.isPopular ? pkg.color : 'rgba(255,255,255,0.05)')}`, padding: '35px 25px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', boxShadow: isActive ? `0 0 50px rgba(76, 175, 80, 0.15)` : (pkg.isPopular ? `0 20px 60px ${pkg.color}22` : '0 10px 40px rgba(0,0,0,0.5)'), overflow: 'hidden' }}>
                                ...
                                {pkg.isRgb && !isActive && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: `linear-gradient(90deg, ${pkg.color}, #00ffff, ${pkg.color})`, backgroundSize: '200% 200%', animation: 'rgbBorder 3s linear infinite' }} />}
                                {isActive && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: '#4caf50' }} />}
                                {pkg.badge && !isActive && <div style={{ position: 'absolute', top: '15px', right: '15px', background: pkg.color, color: '#000', fontSize: '11px', fontWeight: '900', padding: '6px 12px', borderRadius: '10px', letterSpacing: '0.5px', boxShadow: `0 5px 15px ${pkg.color}66` }}>{pkg.badge}</div>}

                                {/* Іконка */}
                                <div style={{ width: '80px', height: '80px', borderRadius: '24px', background: `${pkg.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${pkg.color}44`, marginBottom: '20px', boxShadow: `0 10px 30px ${pkg.color}22` }}>
                                    {pkg.icon}
                                </div>
                                ...
                                <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '10px', marginTop: 'auto' }}>
                                    ...
                                    {/* Кнопка "Що всередині?" */}
                                    <button onClick={() => setSelectedDetailsId(pkg.id)} style={{ width: '100%', padding: '14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', color: 'white', fontSize: '14px', fontWeight: '600', cursor: 'pointer', transition: '0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} className="menu-hover">
                                        <Info size={16} color={pkg.color} /> Що всередині?
                                    </button>

                                    {/* Кнопка Активації або Статус */}
                                    {isActive ? (
                                        <div style={{ width: '100%', padding: '14px', background: 'rgba(76, 175, 80, 0.1)', border: '1px solid rgba(76, 175, 80, 0.3)', borderRadius: '14px', color: '#4caf50', textAlign: 'center', boxShadow: 'inset 0 0 20px rgba(76, 175, 80, 0.05)' }}>
                                            ...
                                            <div style={{ fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', fontWeight: '600', color: '#81c784' }}>
            ...
            {/* 💎 ВНУТРІШНЯ GLASS-МОДАЛКА З ДЕТАЛЯМИ ПАКЕТУ */}
            {detailsPkg && (
                <div style={{ position: 'fixed', inset: 0, zIndex: 10001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(15px)' }} onClick={() => setSelectedDetailsId(null)} />
                    
                    <div className="glass-popup premium-typography" style={{ position: 'relative', width: '100%', maxWidth: '500px', background: 'rgba(20, 20, 25, 0.8)', border: `1px solid ${detailsPkg.color}55`, borderRadius: '28px', padding: '40px', boxShadow: `0 30px 80px rgba(0,0,0,0.9), inset 0 0 40px ${detailsPkg.color}11` }}>
                        
                        <button onClick={() => setSelectedDetailsId(null)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'rgba(255,255,255,0.05)', border: 'none', borderRadius: '50%', padding: '8px', cursor: 'pointer', color: '#888', transition: '0.2s' }} className="menu-hover">
                            <X size={24} />
                        </button>

                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '15px' }}>
                            <div style={{ width: '50px', height: '50px', borderRadius: '16px', background: `${detailsPkg.color}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${detailsPkg.color}66`, flexShrink: 0 }}>
                                {detailsPkg.icon}
                            </div>
                            <h3 style={{ fontSize: '26px', fontWeight: '900', color: 'white', margin: 0, letterSpacing: '-0.5px' }}>Переваги {detailsPkg.title}</h3>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '35px', background: 'rgba(0,0,0,0.3)', padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            {detailsPkg.topFeatures.map((f, i) => (
                                <div key={'top-'+i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                                    ...
                                    <span style={{ color: 'white', fontSize: '15px', fontWeight: '600', lineHeight: '1.4' }}>{f}</span>
                                </div>
                            ))}
                            ...
                            {detailsPkg.features.map((f, i) => (
                                <div key={'f-'+i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                                    ...
                                    <span style={{ color: '#ccc', fontSize: '14px', fontWeight: '500', lineHeight: '1.4' }}>{f}</span>
                                </div>
                            ))}
                        </div>

                        <button onClick={() => setSelectedDetailsId(null)} style={{ width: '100%', padding: '16px', background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '16px', color: 'white', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s' }} className="menu-hover">
                            Зрозуміло, закрити
                        </button>
                    </div>
                </div>
            ...
            {/* 🔥 КАСТОМНА МОДАЛКА ПІДТВЕРДЖЕННЯ ЗАМІНИ */}
            {confirmDialog.isOpen && (
                <div style={{ position: 'fixed', inset: 0, zIndex: 10002, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)' }} onClick={() => setConfirmDialog({ isOpen: false, pkgId: null, price: null })} />
                    
                    <div className="fade-in-up premium-typography" style={{ position: 'relative', background: '#111', border: '1px solid rgba(255, 68, 68, 0.4)', borderRadius: '24px', padding: '30px', maxWidth: '420px', width: '100%', textAlign: 'center', boxShadow: '0 20px 50px rgba(255, 0, 0, 0.2)' }}>
                        ...
                        <h3 style={{ color: 'white', fontSize: '22px', fontWeight: '900', marginBottom: '15px', letterSpacing: '-0.5px' }}>Заміна VIP-статусу</h3>
                        <p style={{ color: '#aaa', fontSize: '15px', lineHeight: '1.5', marginBottom: '30px' }}>
                            У вас вже є активний VIP-статус. Ви дійсно хочете замінити його на новий?<br/><br/>
                            <strong style={{ color: '#ff4444', fontWeight: 'bold' }}>Усі поточні привілеї та залишок часу будуть анульовані без повернення коштів.</strong>
                        </p>
                        
                        <div style={{ display: 'flex', gap: '15px' }}>
                            <button onClick={() => setConfirmDialog({ isOpen: false, pkgId: null, price: null })} style={{ flex: 1, padding: '16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', borderRadius: '16px', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', transition: '0.2s' }} className="menu-hover">
                                Скасувати
                            </button>
                            <button onClick={() => executePurchase(confirmDialog.price, confirmDialog.pkgId)} style={{ flex: 1, padding: '16px', background: 'linear-gradient(90deg, #ff4444, #cc0000)', border: 'none', color: 'white', borderRadius: '16px', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', boxShadow: '0 10px 20px rgba(255,68,68,0.3)', transition: '0.2s' }} className="menu-hover">
                                Так, замінити
                            </button>
                        </div>
                    </div>
