...
import { X, Zap, Camera, CheckCircle2, Plus, CheckSquare, Square, User, Sparkles, MessageCircle, Image, Eye, ChevronRight, ChevronLeft, Check, Send } from 'lucide-react'; 
...
const CreateProfileModal = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        priceFrom: '', priceTo: '', contact: '', contactType: 'Telegram', hairColor: 'blonde', 
    ...
    useEffect(() => {
        if (editingModel) {
            setFormData({
                ...
                priceFrom: editingModel.priceFrom || '', priceTo: editingModel.priceTo || '', contact: editingModel.contact || '', contactType: editingModel.contactType || 'Telegram',
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            const finalProfileData = {
                ...
                gender: formData.gender, bodyType: formData.bodyType, hairColor: formData.hairColor, fetishes: formData.fetishes, contactType: formData.contactType,
            ...
            if (result.success) { toast.success('✅ Анкету збережено!', { id: loadingToast }); loadCatalog(); setShowCreateModal(false); } 
    ...
    const renderStepIndicators = () => {
        ...
        return (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '45px', position: 'relative', padding: '0 5px' }}>
                <div style={{ position: 'absolute', top: '50%', left: '20px', right: '20px', height: '2px', background: 'rgba(255,255,255,0.08)', zIndex: 1, transform: 'translateY(-50%)', borderRadius: '2px' }}></div>
                <div style={{ position: 'absolute', top: '50%', left: '20px', width: `calc(${((step - 1) / 3) * 100}% - 20px)`, height: '2px', background: `linear-gradient(90deg, ${accent}, #ff4081)`, zIndex: 2, transition: 'width 0.5s ease', transform: 'translateY(-50%)', borderRadius: '2px', boxShadow: `0 0 15px ${accent}` }}></div>
    ...
    const activeNetworkData = contactNetworks.find(n => n.id === formData.contactType) || contactNetworks[0];
    const displayPriceText = formData.priceTo ? `${formData.priceFrom} - ${formData.priceTo}` : (formData.priceFrom || "500");

    return (
        <>
            {/* 🔴 СТАТИЧНИЙ ФОН (Вирішує проблему лагів скролу) */}
            ...
            {/* 🔴 НЕЗАЛЕЖНИЙ КОНТЕЙНЕР СКРОЛУ */}
            <div data-lenis-prevent="true" className="custom-scrollbar" style={{ position: 'fixed', inset: 0, zIndex: 4000, overflowY: 'auto', padding: '40px 20px', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', WebkitOverflowScrolling: 'touch' }}>
                
                <div style={{ width: '100%', maxWidth: step === 4 ? '550px' : '750px', background: 'radial-gradient(120% 120% at 50% -20%, #1a1a24 0%, #0a0a0f 100%)', boxShadow: `inset 0 1px 1px rgba(255,255,255,0.1), 0 40px 100px rgba(0,0,0,0.9)`, borderRadius: '36px', padding: '45px', position: 'relative', transition: 'max-width 0.4s cubic-bezier(0.25, 1, 0.5, 1)', margin: 'auto' }}>
                        ...
                        {step === 2 && (
                            <div className="step-animated" style={{ display: 'grid', gap: '30px' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '25px', alignItems: 'end' }}>
                                    ...
                                    <div>
                                        <label style={labelStyle}>Особистий зв'язок</label>
                                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '6px', borderRadius: '20px', display: 'flex', gap: '6px', marginBottom: '15px', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.05)' }}>
                                            {contactNetworks.map(net => {
                                                const isActive = formData.contactType === net.id;
                                                return (
                                                    <button key={net.id} onClick={(e) => { e.preventDefault(); handleChange('contactType', net.id); }} style={{ flex: 1, padding: '12px', background: isActive ? net.color : 'transparent', color: isActive ? 'white' : '#888', border: 'none', borderRadius: '14px', fontWeight: '800', fontSize: '13px', cursor: 'pointer', transition: 'all 0.3s', boxShadow: isActive ? `0 8px 20px ${net.color}66` : 'none' }}>
                                                        {net.id}
                                                    </button>
                                                )
                                            })}
                                        </div>
                                        <input placeholder="Юзернейм або номер телефону" value={formData.contact} onChange={e => handleChange('contact', e.target.value)} style={inputClass} className="premium-input" />
                                    </div>
                                </div>
                            </div>
                        )}

                        {/* 🟢 КРОК 3: ФОТО ТА ПОСЛУГИ */}
                        {step === 3 && (
                            <div className="step-animated" style={{ display: 'grid', gap: '30px' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ maxHeight: '300px', overflowY: 'auto', paddingRight: '15px' }} className="custom-scrollbar">
                            ...
                            <div className="step-animated" style={{ display: 'flex', justifyContent: 'center' }}>
                                <div style={{ width: '100%', maxWidth: '420px', backgroundColor: '#050508', border: `1px solid rgba(255,255,255,0.1)`, borderRadius: '28px', overflow: 'hidden', position: 'relative', boxShadow: `0 30px 60px rgba(0,0,0,0.8)` }}>
                                    ...
                                    <div style={{ padding: '20px 25px 30px' }}>
                                        ...
                                        <button style={{ width: '100%', padding: '18px', background: activeNetworkData.color, borderRadius: '16px', color: 'white', fontWeight: '900', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', fontSize: '16px', letterSpacing: '1px', boxShadow: `0 10px 25px ${activeNetworkData.color}66`, border: 'none', pointerEvents: 'none' }}>
                                            <Send size={20} /> НАПИСАТИ ({formData.contactType})
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            <style dangerouslySetInlineStyle={{__html: `
                /* 🔴 ПРИБИРАЄМО ДЕШЕВІ СИСТЕМНІ СТРІЛОЧКИ У ПОЛЯХ З ЦИФРАМИ */
